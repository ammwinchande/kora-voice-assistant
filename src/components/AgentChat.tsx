...
export const AgentChat = ({ agentId }: AgentChatProps) => {
  ...
  return (
    <div className="flex flex-col items-center gap-6 w-full max-w-2xl mx-auto p-6">
      ...
      {transcript.length > 0 && (
        <div className="w-full bg-black/40 backdrop-blur-xl border border-white/10 rounded-2xl p-6 max-h-96 overflow-y-auto">
      ...
      <div className="relative group">
        {/* Glow Effect */}
        <div className={`absolute -inset-6 bg-gradient-to-r from-blue-500 to-purple-600 rounded-full blur-2xl opacity-30 group-hover:opacity-50 transition-opacity duration-500 ${
          isSpeaking ? 'opacity-60 animate-pulse' : ''
        }`}></div>
        
        <button 
          onClick={status === 'connected' ? handleEndConversation : handleStartConversation}
          disabled={status === 'connecting'}
          className={`relative w-24 h-24 md:w-32 md:h-32 rounded-full flex items-center justify-center transition-all duration-300 transform hover:scale-105 active:scale-95 border-2 ${
            status === 'connected'
